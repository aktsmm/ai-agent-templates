...
Usage:
    ...
    # Single query mode
    python -m content_marketing --query "Write a blog post about AI trends"
...
def main() -> None:
    """Run the content marketing agent."""
    # Load environment variables
    env_path = Path(__file__).parent.parent.parent / ".env"
    load_dotenv(env_path)

    parser = argparse.ArgumentParser(
        description="AI Content Marketing Agent powered by CrewAI"
    )
    parser.add_argument(
        "--query", "-q",
        type=str,
        help="Single content marketing request to process",
    )
    parser.add_argument(
        "--file", "-f",
        type=str,
    ...
    if args.query:
        ...
        # Interactive mode
        print("=" * 60)
        print("  AI Content Marketing Agent")
        print("  Type 'quit' or 'exit' to stop")
        print("=" * 60)

        while True:
            print()
            query = input("Marketer > ").strip()
...
def _process_request(query: str, classify_only: bool = False) -> None:
    """Process a single content marketing request."""
    from content_marketing.crew import classify_request, handle_request
