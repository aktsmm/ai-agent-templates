"""Sales Lead Qualifier Agent - Entry Point.
...
def main() -> None:
    """Run the sales lead qualifier agent."""
    # Load environment variables
    env_path = Path(__file__).parent.parent.parent / ".env"
    load_dotenv(env_path)

    parser = argparse.ArgumentParser(
        description="AI Sales Lead Qualifier Agent powered by CrewAI"
    )
    parser.add_argument(
        "--query", "-q",
        type=str,
        help="Single sales request to process",
    )
    parser.add_argument(
        "--file", "-f",
        type=str,
    ...
    if args.query:
        ...
        # Interactive mode
        print("=" * 60)
        print("  AI Sales Lead Qualifier")
        print("  Type 'quit' or 'exit' to stop")
        print("=" * 60)

        while True:
            print()
            query = input("Sales > ").strip()
...
def _process_query(query: str, classify_only: bool = False) -> None:
    """Process a single sales query."""
    from sales_lead_qualifier.crew import classify_request, handle_request
